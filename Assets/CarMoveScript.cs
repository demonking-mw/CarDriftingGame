...
Name convension note: functions that starts in lower case are smaller help functions, whereas those starting with cap are more significant ones. In another way, those starting in lowercase are ones that will not be refered to other than the one or two specific use case in a larger function.
*/
public class CarMoveScript : MonoBehaviour
{
    ...
    //#########################################################################################
    public float carAccelScale;
    public float carTurnRate;
    public float tyreGrip; // to represent how strong the tyre is
    public float steerInfluence; // factor between steering and tyreGrip
    float steerToDir = 150; // how much steering would point the front wheel at how many degrees;
    public float throttleInfluence; // factor between throttle and Grip of the rear tyre
    public float dragFactor; //use quadratic drag to prevent infinite speed;
    public float driftHelpRate; //how much will the system correct the wheels to help drifting
    public float dragExponent; 
    public float angVelocityFactor;
    public float angVelocityLeniency;//how lenient is getting out of a drift
    public float slippingThrustRate ; //what rate of thrust will be delivered if slipping
    public float slippingThrustSpinRate; //how much will thrust cause the car to spin
    public float slippingSteerRate; //how much steering will have an effect on direction
    public float spinoutAngDrag; //in spinout, how much angular drag are there
    public float driftSteerFactor; //how much steering control are there in drifting;
    public float driftThrustSpinRate; //how much thrust will induce steering in drifting state
    bool isHumanOperated = true;
    public float driftCorrectionRate; //how much will the drift self correct back into going straight
    //###############################################################################################
...
*/
    ...
    void setTractionState(){
        if(frontTraction && rearTraction){//when they are both in traction
            ...
            if(Math.Abs(angularV)/carRigidbody.velocity.magnitude*100 < angVelocityLeniency && Math.Abs(newDriftAngle) < 35 ){//if the car isnt spinning that much and the back wheel lines up with the direction of motion
